...
import br.com.ebr.phonetizer.commons.PhonetizeUtils;
import br.com.ebr.phonetizer.commons.StringUtils;
import br.com.ebr.phonetizer.generator.CodeGenerator;
import br.com.ebr.phonetizer.generator.MultipleCodeGenerator;
...
public class PhonetizerBR {
    ...
    public String fonetizar(String str) {
        str = str.toUpperCase(); //converte tudo p/ maiuscula
        str = StringUtils.removeStrings(str, prepositions); //remove preposições (da, de, do...)
        str = StringUtils.removeStrings(str, titles); //remove os titulos (Sr, Sra, Dr...)
        str = StringUtils.replaceChars(str, accentuationReplacements); //substitui acentos (Á->A, É->E)
        str = StringUtils.strangeRemover(str); //remove coisas não alfanuméricas
        str = StringUtils.replaceStrings(str, letterReplacements); //substitui letras por extenso pela letra mesmo
        str = StringUtils.replaceStrings(str, numberReplacements); //substitui numeros por extenso por digitos
        str = StringUtils.replaceNumbers(str); //soma sequencias de palavras feitas só de digitos
        str = PhonetizeUtils.phonetize(str); //Fonetiza (a mágica fica aqui!)
        str = StringUtils.replaceStrings(str, nameReplacements); //Substitui nomes parecidos
        str = StringUtils.replaceStrings(str, synonimReplacements); //Substitui sinonimos foneticos
        return str;
    }
    
    public String makePhoneticCode(String str) {
    	str = fonetizar(str);
    	return CodeGenerator.randomize(str);
    }

    public ArrayList<String> makeAllPhoneticCodes(String str) {
    	str = fonetizar(str);
